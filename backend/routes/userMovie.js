...
router.get('/recommendations', async (req, res) => {
  ...
  const recommenderUserMovies = await userMovieRepository
    .createQueryBuilder('userMovie')
    .where('userMovie.user_id = :recommender_id', {
      recommender_id: req.query.recommender_id,
    }) //QUERYYYYYYYYY
    .andWhere('userMovie.movie_id NOT IN (:...user_movie_ids)', {
  ...
  //recommenderUserMovies.sort(compareRatingDescending);

  const recommendations = [];

  for (const recommenderUserMovie of recommenderUserMovies) {
    const recommendedFilm = await movieRepository.findOne({
      where: { id: recommenderUserMovie.movie_id },
    });
    recommendedFilm.recommender_rating = recommenderUserMovie.rating;
